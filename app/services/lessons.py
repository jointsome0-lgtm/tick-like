...
def _agents_md(lesson: dict) -> str:
    return _AGENTS_TEMPLATE.format(
        title=lesson["title"],
        status=lesson["status"],
        source=lesson.get("source_url") or "(none)",
    )
...
def _write_brief(path: Path, text: str) -> None:
    """Write a generated brief (AGENTS.md / CLAUDE.md) WITHOUT following a final
    symlink: O_NOFOLLOW makes os.open raise on a symlinked brief instead of
    truncating whatever it points at (the caller treats that error as 'skip the
    lesson workspace')."""
...
def prepare_terminal_workspace(slug: str | None) -> dict | None:
    ...
    try:
        ...
        if not _bundle_dir_is_safe(lesson_dir):  # before any write into it
            return None
        _ensure_bundle_manifest(lesson)
        _write_brief(lesson_dir / AGENTS_FILENAME, _agents_md(lesson))
