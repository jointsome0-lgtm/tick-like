...
# rejected there at capture, so reject them here too.
...
def _zone():
    """The ledger zone: APP_TIMEZONE, or the host's current local tzinfo."""
    return app_tz() or datetime.now().astimezone().tzinfo
...
def parse_period(period: str | None, precision: str) -> tuple[str | None, str | None]:
    ...
    if precision in _RANGE_PRECISIONS:
        ...
        start = _parse_datetime(parts[0].strip(), zone)
        end = _parse_datetime(parts[1].strip(), zone)
        if end <= start:
...
def archive_entry(conn: sqlite3.Connection, entry_id: int) -> None:
    ...
    stays joinable to its events and restorable via unarchive."""
    row = get_entry(conn, entry_id)
    if row is None:
        raise RetroError("unknown retro entry")
    ...
    ts = now_iso()
    with conn:
        conn.execute("UPDATE retro_entries SET archived_at = ? WHERE id = ?",
                     (ts, entry_id))
...
def unarchive_entry(conn: sqlite3.Connection, entry_id: int) -> None:
    ...
    with conn:
        conn.execute("UPDATE retro_entries SET archived_at = NULL WHERE id = ?",
                     (entry_id,))
...
def list_entries(conn: sqlite3.Connection, include_archived: bool = False) -> list[sqlite3.Row]:
    ...
    q += " ORDER BY (period_start IS NULL), period_start DESC, created_at DESC, id DESC"
